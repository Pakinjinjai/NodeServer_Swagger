...
const authorized = (req, res, next) => {
  ...
  if (decodedToken == undefined) {
    return res
      .status(403)
      .json({ message: "ท่านไม่ได้รับอนุญาติให้เข้าถึงข้อมูล" });
  }
...
const isAdmin = (req, res, next) => {
  ...
  if (decodedToken.status != "admins") {
    return res.status(403).json({
      message: "ท่านไม่ได้รับอนุญาติให้เข้าถึงข้อมูล",
      actual: { message: "เฉพาะเจ้าหน้าที่" },
    });
  }
